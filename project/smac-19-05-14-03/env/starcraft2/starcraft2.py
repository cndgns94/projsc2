...
class StarCraft2Env(MultiAgentEnv):
    ...
    def _launch(self):
        ...
        try:
            ...
            self._run_config.data_dir = Path(os.getcwd()) / 'smac' / 'env' / 'starcraft2'
        # HSPARK: 수정 종료
        ...
        self.terrain_height = np.flip(
            np.transpose(np.frombuffer(map_info.terrain_height.data, dtype=np.uint8)
                .reshape(self.map_x, self.map_y)), 1) / 255
        self.pathing_grid = np.flip(
            np.transpose(
                np.unpackbits(   # 8비트 uint 형을 비트 단위로 풀어냄 1 bit --> 1 byte
                    np.frombuffer(map_info.pathing_grid.data, dtype=np.uint8))
